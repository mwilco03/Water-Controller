...
interface WizardState {
  ...
  rtuConfig: {
    ip_address: string;
    station_name: string; // Auto-generated by backend after add
  };
...
export default function WizardPage() {
  ...
  // RTU configuration - only ip_address is user input
  // station_name is auto-generated by backend from IP
  // vendor_id, device_id, and slot_count use backend defaults
  const [rtuConfig, setRtuConfig] = useState({
    ip_address: '',
    station_name: '', // Populated after add from backend response
  });
  ...
  const addRtu = async () => {
    ...
    try {
      ...
      if (res.ok) {
        // Get the auto-generated station_name from response
        const data = await res.json();
        const stationName = data.data?.station_name || data.station_name || `rtu-${rtuConfig.ip_address.replace(/\./g, '-')}`;
        setRtuConfig(prev => ({ ...prev, station_name: stationName }));
        nextStep();
      } else {
        const data = await res.json();
          ...
          {currentStep === 'add-rtu' && (
            <div>
              ...
              <p className="text-hmi-muted text-sm mb-3">
                Enter the IP address of your Remote Terminal Unit (RTU). The RTU should be powered on and connected to the network.
              </p>
              ...
              <div className="space-y-4 max-w-md">
                <div>
                  ...
                  <input
                    ...
                    autoFocus
                  />
                  <p className="text-xs text-gray-500 mt-1">Station name will be auto-generated from IP</p>
                </div>
              </div>
              ...
              <div className="flex justify-between mt-4">
                ...
                <button
                  onClick={addRtu}
                  disabled={loading || !rtuConfig.ip_address}
