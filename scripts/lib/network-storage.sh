...
EOF
    ...
    # Restart networkd
    sudo systemctl restart systemd-networkd 2>&1 | tee -a "$INSTALL_LOG_FILE" || {
...
_configure_ip_networkmanager() {
    ...
    if command -v nmcli >/dev/null 2>&1; then
        ...
        # Activate connection
        sudo nmcli connection up "$conn_name" 2>&1 | tee -a "$INSTALL_LOG_FILE" || {
...
_configure_ip_dhcpcd() {
    ...
    if [ -f "$dhcpcd_conf" ]; then
        sudo cp "$dhcpcd_conf" "${dhcpcd_conf}.backup.$(date +%Y%m%d_%H%M%S)"
    fi
...
EOF
    ...
    # Restart dhcpcd
    sudo systemctl restart dhcpcd 2>&1 | tee -a "$INSTALL_LOG_FILE" || {
...
EOF
    ...
    sudo ifup "$iface" 2>&1 | tee -a "$INSTALL_LOG_FILE" || {
...
tune_network_interface() {
    ...
    if command -v ethtool >/dev/null 2>&1; then
        ...
        sudo ethtool -s "$iface" wol d 2>/dev/null || {
        ...
        sudo ethtool -A "$iface" rx off tx off 2>/dev/null || {
    ...
    sudo ip link set "$iface" mtu 1500 2>/dev/null || {
...
_configure_firewall_ufw() {
    ...
    # Allow API port
    sudo ufw allow "$DEFAULT_API_PORT/tcp" comment "Water-Controller API" 2>&1 | tee -a "$INSTALL_LOG_FILE"

    # Allow PROFINET ports
    sudo ufw allow "$PROFINET_UDP_PORT/udp" comment "PROFINET UDP" 2>&1 | tee -a "$INSTALL_LOG_FILE"
    sudo ufw allow "${PROFINET_TCP_PORT_START}:${PROFINET_TCP_PORT_END}/tcp" comment "PROFINET TCP" 2>&1 | tee -a "$INSTALL_LOG_FILE"

    # Allow SSH with rate limiting if requested
    if [ "$allow_ssh" = "true" ]; then
        sudo ufw limit ssh comment "SSH with rate limiting" 2>&1 | tee -a "$INSTALL_LOG_FILE"
    fi

    # Enable UFW if not already
    if ! sudo ufw status | grep -q "Status: active"; then
        log_info "Enabling UFW..."
        echo "y" | sudo ufw enable 2>&1 | tee -a "$INSTALL_LOG_FILE"
    fi

    # Reload
    sudo ufw reload 2>&1 | tee -a "$INSTALL_LOG_FILE"
...
_configure_firewall_firewalld() {
    ...
    # Allow API port
    sudo firewall-cmd --permanent --add-port="$DEFAULT_API_PORT/tcp" 2>&1 | tee -a "$INSTALL_LOG_FILE"

    # Allow PROFINET ports
    sudo firewall-cmd --permanent --add-port="$PROFINET_UDP_PORT/udp" 2>&1 | tee -a "$INSTALL_LOG_FILE"
    sudo firewall-cmd --permanent --add-port="${PROFINET_TCP_PORT_START}-${PROFINET_TCP_PORT_END}/tcp" 2>&1 | tee -a "$INSTALL_LOG_FILE"

    # Allow SSH if requested
    if [ "$allow_ssh" = "true" ]; then
        sudo firewall-cmd --permanent --add-service=ssh 2>&1 | tee -a "$INSTALL_LOG_FILE"
    fi

    # Reload
    sudo firewall-cmd --reload 2>&1 | tee -a "$INSTALL_LOG_FILE"
...
EOF
    ...
    if [ "$allow_ssh" = "true" ]; then
        sudo tee -a "$nft_file" > /dev/null << EOF
...
EOF
    ...
    sudo chmod 644 "$nft_file"

    # Apply rules
    sudo nft -f "$nft_file" 2>&1 | tee -a "$INSTALL_LOG_FILE" || {
...
_configure_firewall_iptables() {
    ...
    # Allow API port
    sudo iptables -A INPUT -p tcp --dport "$DEFAULT_API_PORT" -j ACCEPT 2>/dev/null

    # Allow PROFINET
    sudo iptables -A INPUT -p udp --dport "$PROFINET_UDP_PORT" -j ACCEPT 2>/dev/null
    sudo iptables -A INPUT -p tcp --dport "${PROFINET_TCP_PORT_START}:${PROFINET_TCP_PORT_END}" -j ACCEPT 2>/dev/null

    # Allow SSH with rate limiting if requested
    if [ "$allow_ssh" = "true" ]; then
        sudo iptables -A INPUT -p tcp --dport 22 -m state --state NEW -m recent --set 2>/dev/null
        sudo iptables -A INPUT -p tcp --dport 22 -m state --state NEW -m recent --update --seconds 60 --hitcount 4 -j DROP 2>/dev/null
        sudo iptables -A INPUT -p tcp --dport 22 -j ACCEPT 2>/dev/null
    fi

    # Log dropped packets
    sudo iptables -A INPUT -m limit --limit 5/min -j LOG --log-prefix "iptables-dropped: " 2>/dev/null
...
configure_tmpfs() {
    ...
    if grep -q "$tmpfs_mount" /etc/fstab 2>/dev/null; then
        ...
        echo "# Water-Controller tmpfs for write endurance" | sudo tee -a /etc/fstab > /dev/null
    ...
    if ! mountpoint -q "$tmpfs_mount" 2>/dev/null; then
        ...
        sudo mount "$tmpfs_mount" 2>&1 | tee -a "$INSTALL_LOG_FILE" || {
            # Try mounting directly
            sudo mount -t tmpfs -o "size=$TMPFS_SIZE,mode=0755" tmpfs "$tmpfs_mount" 2>&1 | tee -a "$INSTALL_LOG_FILE" || {
    ...
    # Set ownership
    sudo chown water-controller:water-controller "$tmpfs_mount" 2>/dev/null || true
