...
TEST(ar_manager_init_null)
{
    ...
    wtc_result_t result = ar_manager_init(NULL, -1, controller_mac, PN_STATION_NAME, PN_VENDOR_ID, PN_DEVICE_ID, "eth0");
    ASSERT_EQ(WTC_ERROR_INVALID_PARAM, result);

    /* NULL MAC address should fail */
    result = ar_manager_init(&manager, -1, NULL, PN_STATION_NAME, PN_VENDOR_ID, PN_DEVICE_ID, "eth0");
    ASSERT_EQ(WTC_ERROR_INVALID_PARAM, result);
